...
class NvidiaProvider:
    def __init__(self, api_key: str, base_url: str, model: str, name: str = "nvidia") -> None:
        self.name = name
        self._model = model
    ...
    async def translate(self, request: TranslationRequest) -> TranslationResult:
        ...
        if not response.choices:
            raise RuntimeError(f"NVIDIA returned no choices for model={self._model}.")
        choice = response.choices[0]
        if choice.message is None:
            raise RuntimeError(f"NVIDIA returned choice with no message for model={self._model}.")
        translated_text = (choice.message.content or "").strip()
        if not translated_text:
            raise RuntimeError("NVIDIA returned empty translation content.")
